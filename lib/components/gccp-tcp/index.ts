...
export class TcpComponent extends Source {
  ...
  constructor(connectionTimeout: number) {
    ...
    outgoing.on('error', e => {
      console.error(
        `TCPComponent ${this.hostname}:${this.port}: error during TCP send, ignoring:`,
        e,
      )
    })
    ...
    outgoing.on('finish', () => {
      if (!_.isNil(this.socket)) this.socket.end()
    })

    incoming.on('error', e => {
      console.error(
        `TCPComponent ${this.hostname}:${this.port}: closing HTTP connection due to incoming error`,
        e,
      )
    ...
    outgoing.write = (msg: Message, callback: any) => {
      ...
      if (_.isNil(this.socket)) {
        ...
        try {
          ...
          if (!_.isNil(this.events)) this.events.emit('error', error)
          return false
        }

        this.socket.on('error', e => {
          if (!_.isNil(this.events)) this.events.emit('error', e)
        })

        // use this to detect the disconnect earlier than 80 seconds to disconnect event
        this.socket.setTimeout(connectionTimeout, () => {
          console.warn(
            `Timeout when connecting to ${this.hostname}:${this.port}`,
          )
