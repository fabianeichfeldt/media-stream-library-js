...
export class HttpComponent extends Source {
    ...
    outgoing.write = (msg: Message, encoding: string, callback: Function) => {
      ...
      if (!this.httpConnection) {
        ...
        this.httpConnection
          ...
          .catch((e: Error) => {
            console.error(
              `HTTPComponent ${this.hostname}:${this.port}: message lost during send:`,
              msg,
              e,
            )
          })
  ...
  // tslint:disable-next-line: newspaper-order
    ...
    if (this.httpConnection) this.httpConnection.end(cb)
    else cb()
  }
  ...
  private createHttpConnection(firstRtspRequest: Buffer): HttpRtspTunnel {
    ...
    const url = firstRtspRequest
      .slice(firstSpace, secondSpace)
      .toString('ascii')
  ...
  private setupComponentEventHandlers() {
    this.outgoing.on('error', (e: any) => {
      console.error(
        `HTTPComponent ${this.hostname}:${this.port}: error during TCP send, ignoring:`,
        e,
      )
    })
    ...
    this.incoming.on('error', (e: any) => {
      console.error(
        `HTTPComponent ${this.hostname}:${this.port}: closing HTTP connection due to incoming error`,
        e,
      )
      this.httpConnection!.end(() => {
        return
  ...
  private initHttpConnection(msg: Message, callback: Function) {
    this.httpConnection!.init()
      .then(() => {
        this.setupHttpConnectionEvents()
        this.httpConnection!.sendCommand(msg.data)
          .then(() => {
      ...
      .catch((e: Error) => {
        ...
        if (this.events) this.events.emit('error', e)
      })
  }

  private setupHttpConnectionEvents() {
    this.httpConnection!.once('disconnect', () => {
      if (this.events) this.events.emit('disconnect')
    })
    this.httpConnection!.on('timeout', () => {
      if (this.events) this.events.emit('timeout')
    })
  ...
  private onData(buffer: Buffer) {
    try {
      if (!this.incoming.push({ data: buffer, type: MessageType.RAW }))
        console.warn(
          `HTTPComponent ${this.hostname}:${this.port}: internal error: not allowed to push more data`,
        )
    } catch (error) {
      if (this.events) this.events.emit('error', error)
    }
  }
}
